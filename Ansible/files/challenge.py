        ...
        if not solves:
            ...
            if status:  # The challenge plugin says the input is right
                ...
                if not current_user.is_admin():
                    chall_solves = Solves.query.filter_by(challenge_id=challenge_id)
                    first_blood = True

                    for solve in chall_solves:
                        if user.id != solve.user_id and get_user_attrs(solve.user_id).type != "admin":
                            first_blood = False
                            break

                    if first_blood:
                        content = ":drop_of_blood: First blood on **" + challenge.name + "** by **" + user.name + "** from **" + team.name + "** :triangular_flag_on_post:"
                        post(getenv("DISCORD_WEBHOOK_URL"), data={'content': content})
                # ---------- FIRST BLOOD ----------
